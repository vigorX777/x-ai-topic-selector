...
**注意**: 所有参数在一个界面中一次性选择，加速配置流程。输出目录固定为当前工作目录下的 `./output`。
...
- **一次性参数选择**: 将原来串行的 5 步参数收集（内容来源 → 评分模式 → 选题范围 → 扫描数量 → 推荐条数）合并为单次交互，用户可以在一个界面中一次性完成所有配置
- **简化配置复用选项**: 已保存配置的选项从 6 个简化为 2 个（"使用上次配置直接运行" / "重新配置全部"）
- **移除输出目录选项**: 输出目录固定为 `./output`（当前工作目录下），减少不必要的选择
- **条件性补充收集**: 仅在必要时（选择 X 列表需要 URL、选择 AI 模式且无保存 Key 需要输入）进行补充询问
- **配置文件精简**: 移除 `outputDir` 字段

**用户体验改进**

- 首次配置从 5+ 次交互减少到 1-2 次
- 有已保存配置时只需 1 次确认即可开始执行
- 保留"上次选择"标记的回填功能
